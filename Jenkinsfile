        ...
        stage('Test Acceptance') {
            steps {
                script {
                    ...
                    sh "curl -i -f -s -X POST -H \"Content-Type: application/json\" -d '{\"name\": \"a\", \"nationality\": \"b\"}' http://localhost:8002/api/v1/casts/ | grep -E '201' || (docker compose logs cast_service && exit 1)"
                    // Test nginx proxying
